...
impl CodeGen {
    ...
    pub(super) fn module_stmts(&mut self, module: &TModule) -> Vec<Stmt> {
        ...
        stmts.push(Stmt::Raw("-- Generated by MATA-LL compiler (https://matall.org/)".into()));
        ...
        stmts.push(Stmt::Local(
            ...
            Some(Expr::lit(lua_quoted_string(env!("CARGO_PKG_VERSION")))),
        ));
        stmts.push(Stmt::Local(
            vec!["__MLLC_COMMIT".into()],
            Some(Expr::lit(lua_quoted_string(env!("MLLC_GIT_COMMIT")))),
        ));
        ...
        for (i, con) in def.constructors.iter().enumerate() {
            ...
            if field_count == 0 {
                if let Some(str_tag) = self.luadict_enum_tag.get(&con.name) {
                    ...
                    let tag_lit = Expr::lit(lua_quoted_string(str_tag));
