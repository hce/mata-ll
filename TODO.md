...
- [ ] Layout: multi-line application-argument continuations must be indented past the *function's* column (`app_col` in parse_expr_app), which is stricter than Haskell (Haskell only needs indentation past the enclosing layout context). So `f = foldr g 0` then `  [1,2,3]` at col 3 is rejected ("Unexpected token at top level"), while indenting past `foldr` works. This is INTENTIONAL/documented, and load-bearing: a naive relaxation (gate on expr_min_indent instead of app_col) makes application greedily swallow subsequent shallow lines and breaks ~dozens of tests (multi-clause defs, do-blocks, GHC cases). A proper fix needs the real layout-context column (per nesting level) to distinguish an application continuation from the next clause/binding/statement — a dedicated layout-engine change, not a quick patch. Workaround for now: indent continuations past the function, or keep on one line. Found while writing examples/scheme.mll and examples/raytracer.mll.
