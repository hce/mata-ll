...
stackTest :: [Integer]
stackTest = toList_ (insert_ 3 (insert_ 2 (insert_ 1 (empty_ :: Stack Integer))))

main :: IO ()
main = do
    -- Stack via typeclass
    let s0 = empty_ :: Stack Integer
    let s1 = insert_ 1 s0
    let s2 = insert_ 2 s1
    let s3 = insert_ 3 s2
    assert (toList_ s3 == [3, 2, 1]) "stack lifo"
    assert (s0 == Stack ([] :: [Integer])) "stack empty eq"
    assert (stackTest == [3, 2, 1]) "stack via function"

    -- Multiple insert/toList cycles
    let s4 = insert_ 10 (insert_ 20 (empty_ :: Stack Integer))
