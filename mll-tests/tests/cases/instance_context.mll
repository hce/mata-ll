...
-- method bodies (the recursive/element `show` calls) and demand `Show Integer`
-- when the instance is used at `Tree Integer`. The instance context used to be
...
main = do
    assert (show (Branch (Leaf 1) (Leaf 2)) == "Branch (Leaf 1) (Leaf 2)")
        "constrained instance at Tree Integer"
    -- Nested: the instance used at Tree (Tree Integer) demands
    -- Show (Tree Integer), which the instance itself provides.
