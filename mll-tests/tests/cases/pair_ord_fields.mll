...
-- instances through Display strings ("Pair a b" vs "Pair Int String"),
...
main = do
    ...
    assert (MkPair (1 :: Int) "b" == MkPair 1 "b") "eq Pair Int String"
    assert (MkPair "a" (2 :: Int) /= MkPair "b" 2) "neq Pair String Int"
    assert (compare (MkPair (1 :: Int) (2 :: Int)) (MkPair 1 3) == LT) "ord second field"
    assert (compare (MkPair (2 :: Int) (0 :: Int)) (MkPair 1 9) == GT) "ord first field wins"

    -- Field methods on user ADT fields.
    assert (compare (MkPair Red (1 :: Int)) (MkPair Blue 0) == LT) "ord Color field"
    ...
    let n1 = MkPair (MkPair (1 :: Int) "x") Red
    let n2 = MkPair (MkPair (1 :: Int) "y") Red
    ...
    assert (MkPair [1 :: Int, 2] (Just Red) == MkPair [1, 2] (Just Red)) "eq list and Maybe fields"
    assert (MkPair [1 :: Int] Nothing /= MkPair [1] (Just Blue)) "neq Maybe field"
