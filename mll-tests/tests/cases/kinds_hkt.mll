...
-- (from the use `t Int` in the method signature, no annotation), with
...
-- `f Int`), and a constrained function dispatching through all of it.
...
unwrapCollapse :: Collapse f => Wrap f -> Int
...
main = do
    ...
    assert (collapse (Right 7 :: Either String Int) == 7) "collapse Either"
    assert (collapse (Left "no" :: Either String Int) == 0) "collapse Left"
    ...
    assert (sum (Right 9 :: Either String Int) == 9) "Prelude Foldable Either"
