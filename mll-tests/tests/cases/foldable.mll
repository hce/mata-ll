...
main = do
    ...
    assert (foldr (\x acc -> x + acc) 1 (Nothing :: Maybe Int) == 1) "foldr Nothing"
    assert (foldl (\acc x -> acc + x) 1 (Just 5) == 6) "foldl Just"
    assert (foldr (\x acc -> x + acc) 1 (Right 5 :: Either String Int) == 6) "foldr Right"
    assert (foldr (\x acc -> x + acc) 1 (Left "e" :: Either String Int) == 1) "foldr Left"
    assert (foldl (\acc x -> acc + x) 1 (Right 5 :: Either String Int) == 6) "foldl Right"
    -- generic functions over lists
    assert (length [1, 2, 3] == 3) "length list"
    assert (null ([] :: [Int])) "null empty list"
    ...
    assert (length (Nothing :: Maybe Int) == 0) "length Nothing"
    assert (null (Nothing :: Maybe Int)) "null Nothing"
    assert (not (null (Just 1))) "null Just"
    assert (elem 5 (Just 5)) "elem Just"
    assert (not (elem 5 (Nothing :: Maybe Int))) "elem Nothing"
    assert (sum (Just 6) == 6) "sum Just"
    assert (product (Nothing :: Maybe Int) == 1) "product Nothing"
    assert (maximum (Just 7) == 7) "maximum Just"
    assert (length (Right 1 :: Either String Int) == 1) "length Right"
    assert (sum (Left "e" :: Either String Int) == 0) "sum Left"
    ...
    assert (foldMap show (Nothing :: Maybe Int) == "") "foldMap Nothing is mempty"
