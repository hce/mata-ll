...
-- "Cannot unify Integer with Bool". Built-in fmap was unaffected.
...
main = do
    ...
    check "int" (myfmap (+ 1) (Just (1 :: Integer))) (Just 2)
    check "bool" (myfmap not (Just True)) (Just False)
    -- A third occurrence changing BOTH sides of the arrow.
    check "int-to-string" (myfmap show (Just (7 :: Integer))) (Just "7")
    -- And a second instance in the same definition too.
    check "box" (case myfmap (* 3) (Box (2 :: Integer)) of Box n -> n) 6
