...
main = do
    ...
    assert (poly_snd (poly (1 :: Integer)) == "1") "sibling before limit"
    ...
    assert (poly [1 :: Integer] == [1]) "poly [Integer]"
    assert (poly ["a"] == ["a"]) "poly [String]"
    assert (poly [True] == [True]) "poly [Bool]"
    assert (poly (Just (1 :: Integer)) == Just 1) "poly Maybe Integer"
    assert (poly (Just "x") == Just "x") "poly Maybe String"
    assert (poly (Just True) == Just True) "poly Maybe Bool"
    assert (poly (1 :: Integer, 2 :: Integer) == (1, 2)) "poly (I, I)"
    assert (poly ("a", "b") == ("a", "b")) "poly (S, S)"
    assert (poly (True, False) == (True, False)) "poly (B, B)"
    assert (poly (1 :: Integer, "a") == (1, "a")) "poly (I, S)"
    assert (poly ("a", 1 :: Integer) == ("a", 1)) "poly (S, I)"
    assert (poly (True, 1 :: Integer) == (True, 1)) "poly (B, I)"
    assert (poly (1 :: Integer, True) == (1, True)) "poly (I, B)"
    assert (poly [[1 :: Integer]] == [[1]]) "poly [[Integer]]"
    -- The sibling's specialization must have survived the purge, and the
    -- early `poly` call sites must still reach a live function.
    assert (poly_snd (2 :: Integer) == "2") "sibling after limit"
    putStrLn "ok"
