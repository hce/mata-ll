...
data HTree = Leaf Integer Integer
           | Node Integer HTree HTree
...
bump :: Integer -> [(Integer, Integer)] -> [(Integer, Integer)]
bump s []            = [(s, 1)]
bump s ((k, c):rest)
  | k == s    = (k, c + 1) : rest
  | otherwise = (k, c) : bump s rest

freqTable :: [Integer] -> [(Integer, Integer)]
freqTable = foldl (\acc s -> bump s acc) []

mkLeaf :: (Integer, Integer) -> HTree
mkLeaf (s, c) = Leaf s c
...
codeTable :: HTree -> [(Integer, [Integer])]
