...
Sum-type \emph{constructors} have a parallel rename for their JSON
tag --- \texttt{Con field-types as "name"} --- described with the JSON
encoding conventions (Section~\ref{sec:conasrename}); it has no
LuaDict counterpart, because a constructor is a positional integer
tag at the Lua boundary, not a name.
...
The rename changes only the JSON tag.  \texttt{show} still prints the
source constructor name (\texttt{show Spades == "Spades"} ---
\texttt{Show} is for debugging, as in Haskell), and construction,
pattern matching, and the runtime representation are untouched.
Unlike the field-level rename, a constructor rename has no Lua-side
counterpart: at the Lua boundary a constructor is a positional
integer tag (an enum value is a plain integer, a fielded variant a
positional table), not a name, so there is no key the rename could
change --- the JSON tag is the only surface where constructors are
named externally.
...
rejected.  The rename requires \texttt{deriving (ToJSON)} or
\texttt{deriving (FromJSON)} (nothing else names constructors
externally), and it is rejected on the constructor of an
