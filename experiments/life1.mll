...
-- render time. Relies on tuple equality and single-line list comprehensions;
-- rows are built with mconcat over String literals (String is opaque, so
-- `++`/`map` over characters do not apply).
module Main where
...
neighbours (x, y) = [ (x + dx, y + dy) | dx <- [-1, 0, 1], dy <- [-1, 0, 1], not (dx == 0 && dy == 0) ]
